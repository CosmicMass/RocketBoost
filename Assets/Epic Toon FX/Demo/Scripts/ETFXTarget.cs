...
public class ETFXTarget : MonoBehaviour
{
    ...
    void SpawnTarget()
    {
		...
		Destroy(respawnEffect, 3.5f); //Removes attached respawn effect after BoatDistance seconds
    }
    ...
    void OnTriggerEnter(Collider col)
    {
        if (col.tag == "Missile") // If collider is tagged as missile
        {
            if (hitParticle)
            {
				...
				Destroy(destructibleEffect, 2f); // Removes hit effect after BoatDistance seconds
